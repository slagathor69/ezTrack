...
def PlayVideo(video_dict, this_trial_df, display_dict,Freezing,mt_cutoff,SIGMA=1):
    """ 
    -------------------------------------------------------------------------------------
        ...
        display_dict:: [dict]
            Dictionary with the following keys:
                ...
                'save_video' : option to save video if desired [bool]
                               Currently, will be saved at 30 fps even if video 
                               is something else
    ...
    experiment = this_trial_df["Experiment"].to_list()[0]
    mouse = this_trial_df["Mouse"].to_list()[0]
    trial = this_trial_df["Trial"].to_list()[0]
    ...
    if display_dict['save_video']==True:
        ...
        fourcc = 0
        writer = cv2.VideoWriter(os.path.join(os.path.normpath(video_dict['dpath']), 'results', f'{experiment}-Mouse{mouse}-Trial{trial}.avi'), 
                         fourcc, 30.0, (width, height), isColor=False)
...
def SaveData(video_dict,Motion,Freezing,mt_cutoff,FreezeThresh,MinDuration):
    ...
    DataFrame.to_csv((os.path.splitext(video_dict['dpath'])[0] + '/results/' + os.path.splitext(video_dict['file'])[0] + '-FreezingOutput.csv'), index = False)
